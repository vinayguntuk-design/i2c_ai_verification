...
# ================================================================
# GLOBAL CUMULATIVE COVERAGE
# Keeps coverage across all simulation runs
# ================================================================
...
def update_tb_profile(profile: str) -> bool:
    ...
    with open(TB_FILE, 'w', encoding='utf-8', errors='ignore') as f:
        f.write(new_content)
...
def find_vivado() -> Optional[str]:
    ...
    # If user supplied environment variable VIVADO_BIN, prefer that
    env_path = os.environ.get("VIVADO_BIN")
    if env_path:
        xvlog_win = os.path.join(env_path, "xvlog.bat")
        xvlog = os.path.join(env_path, "xvlog")
        if os.path.exists(xvlog_win) or os.path.exists(xvlog):
            print(f"  [SIM] Using VIVADO_BIN from environment: {env_path}")
            return env_path

    # Common Vivado install locations (include AMD layout)
    search_paths = [
        # AMD / Xilinx Windows
        ...
        if compile_result.returncode != 0:
            ...
            for line in compile_result.stderr.splitlines()[:20]:
                if line.strip():
                    print(f"    {line}")
            combined = "\n".join(all_output)
            with open(log_file, 'w', encoding='utf-8', errors='ignore') as f: f.write(combined)
...
def strategy_rl(cov: CoverageData, history: list) -> str:
    ...
    with open(Q_TABLE_FILE, 'w', encoding='utf-8', errors='ignore') as f:
...
def print_progress_chart(history, current_pct):
...
Examples:
  ...
  python3 ai/vivado_ai.py --strategy rl --runs 8
    ...
    for run_num in range(args.runs):
        ...
        # --------------------------------------------------------
        # MERGE CUMULATIVE COVERAGE
        # Coverage should NEVER decrease across runs
        # --------------------------------------------------------

        global GLOBAL_COVERAGE

        GLOBAL_COVERAGE.addr_reserved = max(
            GLOBAL_COVERAGE.addr_reserved,
            cov.addr_reserved
        )

        GLOBAL_COVERAGE.addr_low = max(
            GLOBAL_COVERAGE.addr_low,
            cov.addr_low
        )

        GLOBAL_COVERAGE.addr_mid = max(
            GLOBAL_COVERAGE.addr_mid,
            cov.addr_mid
        )

        GLOBAL_COVERAGE.addr_high = max(
            GLOBAL_COVERAGE.addr_high,
            cov.addr_high
        )

        GLOBAL_COVERAGE.rw_write = max(
            GLOBAL_COVERAGE.rw_write,
            cov.rw_write
        )

        GLOBAL_COVERAGE.rw_read = max(
            GLOBAL_COVERAGE.rw_read,
            cov.rw_read
        )

        GLOBAL_COVERAGE.ack = max(
            GLOBAL_COVERAGE.ack,
            cov.ack
        )

        GLOBAL_COVERAGE.nack = max(
            GLOBAL_COVERAGE.nack,
            cov.nack
        )

        GLOBAL_COVERAGE.data_zero = max(
            GLOBAL_COVERAGE.data_zero,
            cov.data_zero
        )

        GLOBAL_COVERAGE.data_ones = max(
            GLOBAL_COVERAGE.data_ones,
            cov.data_ones
        )

        GLOBAL_COVERAGE.data_alt = max(
            GLOBAL_COVERAGE.data_alt,
            cov.data_alt
        )

        GLOBAL_COVERAGE.total_txn += cov.total_txn

        # Use cumulative coverage from now on
        cov = GLOBAL_COVERAGE
        ...
        if current_pct >= args.target:
            ...
            for h in history:
                tick = "OK" if h["pct"] >= args.target else " "
        ...
        if len(history) >= 3:
            ...
            if same_cov:
                ...
                print("  [AI] Coverage stagnated.")
                print("  [AI] Forcing exploration profile.")
                ...
                profile = random.choice(explore_profiles)

            else:

                profile = pick_next_profile(
                    args.strategy,
                    cov,
                    history
                )
