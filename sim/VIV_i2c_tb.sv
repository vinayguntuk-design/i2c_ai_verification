...
//=============================================================
// CLASS 2: DRIVER
// Takes a transaction from the generator mailbox
// Applies it to the DUT (master) input pins
// Sends completed transaction to scoreboard
//=============================================================
class i2c_driver;

    // Virtual interface = runtime connection to actual signals
    virtual interface i2c_if vif;

    // Mailboxes = thread-safe queues
    mailbox #(i2c_transaction) from_gen;   // receives from generator
    mailbox #(i2c_transaction) to_scb;     // sends to scoreboard
    ...
    task run();
        i2c_transaction txn;
        // Start with DUT inputs at safe idle state
        vif.start   <= 1'b0;
        vif.rw      <= 1'b0;
        vif.addr    <= 7'd0;
        vif.data_in <= 8'd0;
        ...
        forever begin
            // Wait until generator puts a transaction in the mailbox
            from_gen.get(txn);
            txn.print("DRV");
            ...
            // Timeout after 5000 cycles to prevent hanging
            begin
                int timeout = 0;
...
module i2c_tb;
    ...
    initial begin
        ...
        fork
            drv.run();    // driver loop
            mon.run();    // monitor loop
            scb.run();    // scoreboard loop
            cov.run();    // coverage loop
            gen(profile); // generator — produces transactions
        join_any

        // Allow last transactions to complete
    ...
    task automatic gen(string profile);
        ...
        for (int i = 0; i < `NUM_TRANSACTIONS; i++) begin
            ...
            // Apply AI profile — sets constraint flags
            txn.apply_profile(profile);

            // Randomize the transaction fields
            if (!txn.randomize()) begin
                $display("[GEN] Randomization failed on txn %0d, using default", i);
                txn.addr = `SLAVE_ADDR;
                txn.data = 8'hAB;
                txn.rw   = 1'b0;
            end

            // For "random" profile on run 0:
            // use the real slave address most of the time
            // so we get ACK responses and build baseline coverage
            if (profile == "random" && ($urandom % 3 != 0))
                txn.addr = `SLAVE_ADDR;

            gen_to_drv.put(txn);  // send to driver
        end

        // Wait for last transaction to complete
        repeat(1000) @(posedge clk);
        $display("[GEN] Finished sending %0d transactions", `NUM_TRANSACTIONS);
    endtask

    // ── Watchdog ─────────────────────────────────────────────
    // Prevents simulation from hanging forever
    initial begin
        #10_000_000;
        $display("[WATCHDOG] Timeout — forcing simulation end");
        $finish;
    end
